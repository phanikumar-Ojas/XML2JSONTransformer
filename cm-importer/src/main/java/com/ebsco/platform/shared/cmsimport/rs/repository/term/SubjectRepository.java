...
@Log4j2
public class SubjectRepository extends BasicTermRepository {

    public SubjectRepository(DatabaseClient databaseClient) {
    ...
    public String query() {
        return """
                SELECT term, term_type, an from t_build_lookup_editorial where an in (select mfs_an
                    ...
                    WHERE ta.do_not_use = 'FALSE') and term_type='Subject'
                """;
    }
    ...
    public BasicTerm create(String termTitle) {
        return new SubjectTerm(termTitle);
    }
}

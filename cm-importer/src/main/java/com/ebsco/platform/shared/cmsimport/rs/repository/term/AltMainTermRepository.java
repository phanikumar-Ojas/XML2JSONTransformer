...
public class AltMainTermRepository extends BasicTermRepository {

    public AltMainTermRepository(DatabaseClient databaseClient) {
    ...
    public String query() {
        return """
                SELECT term, term_type, ui from t_adhoc where ui in (select article_id
                    ...
                    WHERE ta.do_not_use = 'FALSE') AND term_type='alt-main'
                """;
    }
    ...
    public BasicTerm create(String termTitle) {
        return new AltMainTerm(termTitle);
    }
}
